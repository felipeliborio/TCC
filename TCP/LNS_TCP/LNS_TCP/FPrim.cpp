...
FGraph FPrim::run(int origin, double alpha, bool classify, bool invalidateOverLimit)
{
	...
	if (invalidateOverLimit && (lCount > graph.maxL || rCount > graph.maxR) || terminalsLeft.size() > 0) {
...
{
	int alphaK = (int)(alpha * priority_queue.end);
	int toExtract = (alphaK > 1) ? eng() % alphaK : 0;
	...
	if (ta.size() > 0) { toExtract = ta[eng() % ta.size()];	}
	else if (tb.size() > 0) { toExtract = tb[eng() % tb.size()]; }
	else if (tc.size() > 0 && graph.maxR - rCount < graph.maxL - lCount) { toExtract = tc[eng() % tc.size()]; }
	else if (td.size() > 0 && graph.maxL - lCount < graph.maxR - rCount) { toExtract = td[eng() % td.size()]; }
