...
void FLNS::runMainLoop(FGraph& bestSolution, FGraph& newSolution,
	...
	int max_phase_1, int max_phase_2, bool LRFocus)
{
	bool phase = false;
	int unenhanced = 0;
	bool done = false;
	...
	while (!done) {
		...
		if (!LRFocus) {
			...
			if (newSolution.length > bestSolution.length) {
				...
				if (newSolution.length < bestSolution.length) {
					auto ElapsedTime = std::chrono::high_resolution_clock::now() - lastStartTime;
					lastRunTimeToBest = (long)std::chrono::duration_cast<std::chrono::milliseconds>(ElapsedTime).count();
					unenhanced = 0;
					phase = false;
				}
				bestSolution = newSolution;
			}
			if (!phase && unenhanced > max_phase_1) {
				phase = true;
				unenhanced = 0;
			}
			else if (phase && unenhanced > max_phase_2) {
				done = true;
			}
		}
		else {
			...
			if (
				((newSolution.rCount <= graph.maxR && newSolution.lCount <= bestSolution.lCount) ||
				(newSolution.lCount <= graph.maxL && newSolution.rCount <= bestSolution.rCount)) &&
				newSolution.length < bestSolution.length ||
				(newSolution.rCount <= bestSolution.rCount && newSolution.lCount <= bestSolution.lCount && 
				newSolution.lCount + newSolution.rCount < bestSolution.lCount + bestSolution.rCount)
				) {
				if (newSolution.length < bestLength.length) {
					bestLength = newSolution;
					unenhanced = 0;
					if (bestLength.lCount < graph.maxL && bestLength.rCount < graph.maxR) {
						auto ElapsedTime = std::chrono::high_resolution_clock::now() - lastStartTime;
						lastRunTimeToBest = (long)std::chrono::duration_cast<std::chrono::milliseconds>(ElapsedTime).count();
						timed = true;
					}
			...
			if (unenhanced > max_phase_1 || (unenhanced > max_phase_2 && 
				(bestSolution.rCount <= graph.maxR && bestSolution.lCount <= graph.maxL))) {
				if (bestLength.lCount < graph.maxL && bestLength.rCount < graph.maxR) {
					bestSolution = bestLength;
				}
				done = true;
			}
		}
...
FGraph FLNS::run(int limit)
{
	...
	FPrim bs(this->graph);
	FGraph bestSolution = bs.run(graph.terminals[distrTerm(eng)], 0, false, false);
	...
	runMainLoop(bestSolution, newSolution, eng, distrDest, distrVtx, 5 * limit, limit);
	if (bestSolution.lCount > graph.maxL || bestSolution.rCount > graph.maxR) {
		runMainLoop(bestSolution, newSolution, eng, distrDest, distrVtx, 20 * limit, 5 * limit, true);
	}

	auto ElapsedTime = std::chrono::high_resolution_clock::now() - lastStartTime;
	lastRunTime = (long)std::chrono::duration_cast<std::chrono::milliseconds>(ElapsedTime).count();
...
void FLNS::printLastResult(bool printGraph, std::string instance)
{
	if (printGraph) {
		std::cout << instance << "\n\ttime: " << lastRunTime << "\t";
		std::cout << instance << "\n\ttime to best: " << lastRunTimeToBest << "\t";
		...
		std::cout << ph << "\tv\te\tt\tlen\ttime\tt to best\n";
...
void FLNS::runBattery(int runs, std::string instance, int limit)
{
	std::vector<int> lengthVec, runTimeVec, timeToBestVec, lVec, rVec;
	...
	for (int i = 0; i < runs; i++) {
		...
		if (lastRunGraph.length != INT32_MAX) {
			...
			timeToBestVec.push_back(lastRunTimeToBest);
	...
	double lengthAvg = -1, lengthMedian = -1, timeAvg = -1, timeToBestAvg = -1;
	...
	if (lengthVec.size() > 0) {
		...
		timeToBestAvg = vecAvg(timeToBestVec);
	...
	std::cout << instance << "\t" << lengthMin << "\t" << lengthMax << "\t" <<
		lengthAvg << "\t" << lengthMedian << "\t" << timeAvg << "\t" << timeToBestAvg <<
