	...
	FHeap& priority_queue, std::vector<int>& parentVec, std::vector<int>& degreeVec)
{
	...
	for (int i = 0; i <= alphaK; i++) {
		switch (classifyVertex(parentVec[priority_queue.elements[i].vertex], degreeVec)) {
		case TA:
			ta.push_back(i);
			break;
		case TB:
...
FGraph FPrim::buildGraph(std::vector<int>& parentVec, std::vector<int>& distanceVec, int lCount, int rCount)
{
	FGraph output;
...
int FPrim::checkOrigin(int origin, FGraph& graph)
{
	int aux = origin;
...
FGraph FPrim::run(int origin, double alpha)
{
	...
	FGraph rGraph = this->graph;

	auto terminalsLeft = rGraph.terminals;
...
run_start:
	origin = checkOrigin(origin, rGraph);//in case it is a vertex of degree 0
	priority_queue = FHeap();

	distanceVec.clear();
	distanceVec.resize(rGraph.vertexCount + 1, INT32_MAX);
	parentVec.clear();
	parentVec.resize(rGraph.vertexCount + 1, -1);
	inMST.clear();
	inMST.resize(rGraph.vertexCount + 1, false);
	...
	while (priority_queue.end != -1)
	{
		...
		degreeVec.clear();
		degreeVec.resize(rGraph.vertexCount + 1, 0);
		...
		int toExtract = pickNextVertexFromHeap(alpha, eng, lCount, rCount, ta, tb, tc, td, priority_queue, parentVec, degreeVec);

		auto lrCount = countLAndR(degreeVec);
		lCount = lrCount.first;
		rCount = lrCount.second;

		if (lCount > rGraph.maxL ||rCount > rGraph.rCount) {
			if (redone < 10){
				redone++;
				//if it's broken, go back
				FGraph reached = buildGraph(parentVec, distanceVec, lCount, rCount);
				reached.simplifyInstance();

				for (int i = 1; i < rGraph.graph.size(); i++) {
					if (reached.graph[i].size() > 0) {
						for (int j = 0; j < rGraph.graph[i].size(); j++) {
							rGraph.graph[rGraph.graph[i][j].first].clear();
						}
					}
				}
				for (int i = 1; i < reached.graph.size(); i++) {
					for (int j = 0; j < reached.graph[i].size(); j++) {
						rGraph.graph[i].push_back(std::make_pair(reached.graph[i][j].first, reached.graph[i][j].second));
						rGraph.graph[reached.graph[i][j].first].push_back(std::make_pair(i, reached.graph[i][j].second));
					}
				}
		...
		if (rGraph.bIsTerminal[vertex1]) {
			auto del = std::find(terminalsLeft.begin(), terminalsLeft.end(), vertex1);
			if (del != terminalsLeft.end()) {
				terminalsLeft.erase(del);
				redone = 0;//there was real progress
			}
		}

		for (int i = 0; i < rGraph.graph[vertex1].size(); i++)
		{
			int vertex2 = rGraph.graph[vertex1][i].first;
			int distance = rGraph.graph[vertex1][i].second;
	...
	lastRunGraph = buildGraph(parentVec, distanceVec, lCount, rCount);
	...
	if (terminalsLeft.size() > 0 || lCount > rGraph.maxL || rCount > rGraph.maxR) {
