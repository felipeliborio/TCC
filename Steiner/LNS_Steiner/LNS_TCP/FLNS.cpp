	...
	for (int i = 0; i < runs; i++) {
		...
		if (newSolution.length > bestSolution.length) {
			...
			if (newSolution.length < bestSolution.length) {
				...
				lastRunTimeToBest = (int)std::chrono::duration_cast<std::chrono::milliseconds>(ElapsedTime).count();
			}
...
FGraph FLNS::run()
{
	...
	runMainLoop(bestSolution, newSolution, eng, distrDest, distrVtx, 40000 + graph.vertexCount);
