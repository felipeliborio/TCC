...
const int Disconnected = -2147483648;
...
TEdges LNS_Steiner(FInstance & Instance, TEdges & Solution)
{
	...
	while (NoEnhancementIt < 500 + Instance.TerminalCount) {//Instance.VertexCount / 2) {
		ExpandNeighborhood(Neighborhood, InstEdgeMatrix, Instance.EdgeCount, NewSolution,((rand() % 2 + 2) / 20.0));
